...
function FillThePageContents(data) {
    ...
    if (data['msg'].length === 0) {
        ...
        for (let i = 0; i < dataLength; i++) {
            contentBlock.innerHTML += `
                                <div class="relative border mb-3 mt-4 mx-2 h-40 md:h-60 lg:h-80 w-[40%] md:w-[20%] lg:w-[20%] xl:w-[15%] rounded-2xl hover:scale-105 hover:shadow-[0_0_15px_2px_rgba(255,255,255,0.8)] transition-transform duration-300"> 
                                    <div id="banProductId${i}" class="hidden">${data['msg'][i]['BanPID']}</div>    
                                    ...
                                    <div class="relative z-10 w-full h-full flex flex-col items-center justify-center">
                                        ...
                                        <button id="banProductBtn${i}" class="border bg-white/10 hover:bg-green-500/30 px-4 py-1 mt-[10%] font-semibold rounded-full">Restore</button>
                                    </div>
                                </div>`;
        }
        ...
        for (let i = 0; i < dataLength; i++) {
            document.getElementById(`banProductBtn${i}`).addEventListener('click', () => {
                let BanPID = document.getElementById(`banProductId${i}`);
                restoreProductActivationFunction(BanPID);
            });
        }
    }
}
